...
export default function MegaMenuNavbar() {
  ...
  useEffect(() => {
    ...
    const loadMenuTags = async () => {
      try {
        const resp = await fetch(`${API}/api/admin/top-banner`, {
          cache: "no-store",
        });
    ...
    <div className="bg-white border-b border-gray-200 relative">
      <div className="max-w-7xl mx-auto relative">
        ...
        <nav>
          ...
          >
            <svg
              className="w-3 h-3 md:w-4 md:h-4 text-rose-700"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
            ...
            {menuTags.map((tag, idx) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="inline text-[10px] sm:text-[12px]">
                    {tag.name}
                  </span>
                </Link>
              );
            ...
            {categories.map((category) => {
              ...
              return (
                <div
                  ...
                  className="shrink-0"
                  onMouseEnter={() =>
                    hasSubcategories && handleMouseEnter(category._id)
                  }
        ...
        {hoveredCategory && (
          ...
          >
            <div className="px-6 py-6">
              <div
                className="grid grid-cols-3 lg:grid-cols-4 gap-x-6 gap-y-4"
                style={{ maxHeight: "70vh", overflowY: "auto" }}
              >
                {getCategorySubcategories(hoveredCategory).map(
                  (subcategory) => {
                    const subSubcats = getSubSubcategories(subcategory._id);
                    const hoveredCategorySlug =
                      categories.find((c) => c._id === hoveredCategory)?.slug ||
                      "";
