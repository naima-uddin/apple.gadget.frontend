...
export default function SearchResultsClient() {
    ...
    (async () => {
      try {
        const res = await fetch(
          `${API}/api/products?q=${encodeURIComponent(query.trim())}&limit=100`,
        );
  ...
  const filtered = useMemo(() => {
    ...
    return allProducts.filter((p) => {
      const price = getDisplayPrice(p).price;
      if (price < priceRange[0] || price > priceRange[1]) return false;
      if (expandedSubIds.size > 0 && !expandedSubIds.has(String(p.categoryId)))
        return false;
  ...
  const sorted = useMemo(() => {
    ...
    switch (sortOption) {
      ...
      case "expressDelivery":
        list.sort(
          (a, b) => (hasExpress(b) ? -1 : 1) - (hasExpress(a) ? -1 : 1),
        );
        break;
      default:
        break;
  ...
  return (
    <div className="max-w-7xl mx-auto px-2 md:px-4 lg:px-6 py-8">
      ...
      <h1 className="text-2xl font-semibold text-center mb-8 text-gray-800">
        Search result for &ldquo;<span className="font-bold"> {query} </span>
        &rdquo;
      </h1>
