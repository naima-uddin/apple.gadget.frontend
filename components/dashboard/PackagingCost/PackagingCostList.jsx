...
export default function PackagingCostList() {
  ...
  const loadItems = async () => {
    ...
    try {
      ...
      if (!resp.ok) throw new Error(body.error || "Failed to load packaging costs");
      setItems(body.items || []);
    } catch (err) {
      alert(err.message || "Failed to load packaging costs");
    } finally {
      setLoading(false);
    }
  };
  ...
  const saveItem = async (e) => {
    ...
    try {
      const resp = await fetch(
        editingId
          ? `${API}/api/admin/packaging-costs/${editingId}`
  ...
  const deleteItem = async (id) => {
    if (!confirm("Delete this packaging cost?")) return;
    try {
      const resp = await fetch(`${API}/api/admin/packaging-costs/${id}`, {
        method: "DELETE",
        credentials: "include",
      });
      const body = await resp.json();
