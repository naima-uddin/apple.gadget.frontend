...
export default function MostSearched() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `${API}/api/analytics/most-searched?limit=${limit}`,
        {
          credentials: "include",
        },
      );
  ...
  const handleDelete = async (item) => {
    if (!confirm(`Delete search term "${item.term}"? This cannot be undone.`))
      return;
    setDeletingId(item._id);
    try {
      const res = await fetch(
        `${API}/api/analytics/most-searched/${item._id}`,
        {
          method: "DELETE",
          credentials: "include",
        },
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-[#1F2937]">Most Searched</h1>
          <p className="text-sm text-gray-500 mt-0.5">
            Top search terms entered by visitors (guests &amp; logged-in users).
          </p>
        </div>
