...
export default function GoogleTagManagerForm() {
  ...
  useEffect(() => {
    fetch(`${API}/api/admin/settings`, { credentials: "include" })
      .then((r) => r.json())
      .then((b) => {
        if (b.settings?.googleTagManager) {
          setCfg((prev) => ({
            ...prev,
            ...b.settings.googleTagManager,
            installed: true,
          }));
        }
      })
  ...
  const handleSave = async () => {
    ...
    try {
      const resp = await fetch(`${API}/api/admin/settings`, {
        ...
        credentials: "include",
        body: JSON.stringify({ googleTagManager: cfg }),
      });
  ...
  if (loading)
    return (
      <div className="flex items-center justify-center gap-2 py-16 text-center text-gray-400">
    ...
    <div className="max-w-2xl mx-auto">
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-6 space-y-5">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h2 className="font-semibold text-gray-900">GTM Configuration</h2>
        </div>
        ...
        <div>
          ...
          <p className="mt-1.5 text-xs text-gray-400">
            Find your Container ID in Google Tag Manager under Admin → Container
            Settings.
          </p>
        </div>
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-lg px-4 py-3 text-amber-900 text-xs">
          ...
          <ol className="list-decimal pl-4 space-y-1">
            <li>
              Go to <strong>tagmanager.google.com</strong> and sign in
            </li>
