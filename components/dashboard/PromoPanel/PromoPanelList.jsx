  ...
  return (
    <div className="bg-white border-2 border-orange-200 rounded-2xl p-5 space-y-4">
      ...
      <p className="text-sm text-gray-500">
        Search and select one product to feature as the Deal of the Day on the
        homepage.
      </p>
      ...
      {loading ? (
        ...
        <div className="flex items-center gap-3 p-3 bg-orange-50 border border-orange-200 rounded-xl">
          <Image
            ...
            width={56}
            height={56}
            className="w-14 h-14 object-contain rounded-lg border bg-white shrink-0"
          />
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm text-orange-600 font-medium">
              Currently selected as Deal of the Day
            </p>
          </div>
          <button
            onClick={handleClear}
        ...
        <p className="text-sm text-gray-400 italic">
          No product selected — search below to pick one.
        </p>
      )}
            ...
            {results.map((p) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="text-orange-500 text-xs font-semibold shrink-0">
                    Select
                  </span>
                </button>
              );
...
export default function PromoPanelList() {
  ...
  const handleToggleActive = async (panel) => {
    try {
      ...
      setPanels((prev) =>
        prev.map((p) =>
          p._id === panel._id ? { ...p, isActive: !p.isActive } : p,
        ),
      );
    } catch {
  ...
  const handleDelete = async (id) => {
    ...
    setDeleting(id);
    try {
      const resp = await fetch(`${API}/api/admin/promo-panels/${id}`, {
        method: "DELETE",
        credentials: "include",
      });
      if (!resp.ok) throw new Error("Failed");
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">
              Left-side panel + product carousel for the Popular Picks section.
              The first active panel is shown.
            </p>
          </div>
          <button
