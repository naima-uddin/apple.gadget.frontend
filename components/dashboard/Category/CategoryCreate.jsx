...
export default function CategoryCreate({ onSuccess }) {
  ...
  const handleFile = async (file) => {
    ...
    try {
      ...
      setImages((imgs) =>
        imgs.map((img) => (img.__local && img.url === preview ? asset : img)),
      );
      try {
      ...
      setImages((imgs) =>
        imgs.filter((i) => !(i.__local && i.url === preview)),
      );
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (images.length > 0)
        payload.images = images.filter((img) => !img.__local);
        ...
        <div>
          ...
          <div className="flex gap-3 flex-wrap mt-2">
            ...
            <button
              ...
              className="w-24 h-24 flex flex-col items-center justify-center border border-dashed border-gray-300 rounded-lg cursor-pointer text-xs text-gray-500 bg-gray-50 hover:bg-gray-100 gap-1"
            >
              <span className="text-lg">🖼</span>Library
            </button>
          </div>
          <p className="text-xs text-gray-500 mt-2">
            Recommended: square image (e.g. 800×800).
          </p>
          <MediaPicker
            open={showPicker}
            onSelect={(asset) => {
