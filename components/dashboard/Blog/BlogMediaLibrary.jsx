...
export default function BlogMediaLibrary({
  ...
  const [folder, setFolder] = useState("appleProduct/blog");
  ...
  const loadFolders = useCallback(async () => {
    try {
      ...
      setFolders(["appleProduct/blog", ...blogFolders]);
    } catch (e) {
      console.error(e);
    }
  }, [API]);
        ...
        {items.map((item) => (
          <div
            ...
            className={`relative aspect-square rounded-lg overflow-hidden cursor-pointer group border-2 transition-all ${
              selected.has(item.public_id)
                ? "border-gray-500"
                : "border-transparent"
            } ${onSelect ? "hover:border-green-400" : "hover:border-gray-400"}`}
          ...
          >
            ...
            {showSelection && selected.has(item.public_id) && (
              <div className="absolute top-2 right-2 w-6 h-6 bg-gray-800 text-white rounded-full flex items-center justify-center text-sm">
                ✓
              </div>
            )}
